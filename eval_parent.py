...
parser.add_argument('--cooccurrence_counts', type=str, default=None)
parser.add_argument('--entailment_fn', type=str, default="overlap")
parser.add_argument('--lambda_weight', type=float, default=None)
parser.add_argument('--smoothing', type=float, default=0.00001)
...
def _table_reader(table_file):
  ...
  with io.open(table_file) as f:
    for line in f:
      ...
      table = [
          [member.split() for member in entry.split("|||")] for entry in entries if entry.strip()
      ]
      yield table
...
def _mention_probability(table_entry, sentence, smoothing=0.00001):
...
def main():
  ...
  if FLAGS.entailment_fn == "cooccurrence":
    ...
    with open(FLAGS.cooccurrence_counts) as f:
